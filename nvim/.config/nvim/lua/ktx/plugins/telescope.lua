return {
	{
		...
		config = function()
			...
			local function projects(opts)
				opts = opts or {}
				local command = "ls -1d ~/Developer/*/*/*/ ~/Developer/*/jasonwoodland/dotfiles/*/"
			...
			function terminal_buffers()
				...
				local terminal_buffers = vim.tbl_filter(function(val)
			...
			noremap("n", "<space>ia", function()
				require("telescope").extensions.gh.issues({ assignee = "jasonwoodland" })
			end)
			...
			-- noremap("n", "<space>n", require("telescope.builtin").current_buffer_fuzzy_find)
			vim.api.nvim_create_autocmd("User", {
				pattern = "TelescopePreviewerLoaded",
				callback = function()
					-- “0” refers to the preview window, so this sets `number` there
			...
			vim.keymap.set("n", "<space>n", function()
				...
				-- Override entry_maker so that:
				-- • `display` is just the raw line text (no "42: foo()")
				-- • `highlights` comes from opts.line_highlights (built-in)
				opts.entry_maker = function(entry)
					local row = entry.lnum
					return {
						valid = true,
						value = entry, -- pressing <CR> jumps to this entry.bufnr,row⋯
						display = entry.text, -- **no** "23: foo()" prefix, just "foo()"
						ordinal = entry.text,
						bufnr = entry.bufnr,
						lnum = entry.lnum,
						col = 0,
						filename = entry.filename,
						highlights = opts.line_highlights and opts.line_highlights[row] or nil,
					}
				end
				require("telescope.builtin").current_buffer_fuzzy_find(opts)
			end, { desc = "Telescope: buffer filter (no linenr, TS highlights, prompt bottom)" })
		end,
	},
